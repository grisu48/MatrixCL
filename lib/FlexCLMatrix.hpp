...
protected:
	...
	/** Read file and parse macros 
	 * @return The read and parsed program code
	 * @throws const char* If something happens while reading
	 * */
	std::string readProgram(std::string filename);
	...
	/**
	 * @return the "$OCL_MACROS" replacement string
	 */
	virtual std::string readMacro(void);
