...
double Matrix3d::l2Norm(bool includeRim) {
	const double dotProd = this->dotProduct(this, includeRim);
	return sqrt(dotProd);
}
...
std::string CLMatrix_d::readMacro(void) {
	throw "OCL Macro not defined";
}

std::string CLMatrix_d::readProgram(std::string filename) {
	return readProgram(filename.c_str());
}
std::string CLMatrix_d::readProgram(const char* filename) {
	if (strlen(filename) <= 0) throw "Empty filename";
	...
	try {
		...
		while (getline(in,line)) {
			if(line == "$OCL_MACROS") {		// Macro replacement
				buf << this->readMacro();
			} else
				buf << line;
			buf << endl;
		...
		if(in.bad()) throw "IO error";
		else if(!in.eof()) throw "Format error";
...
void CLMatrix_d::initializeContext() {
	...
	try {
		string program = this->readProgram(_FLEXCL_MATRIX_KERNEL_FILENAME);
		this->_program = this->_context->createProgramFromSource(program);
	} catch (const char* msg) {
		// Something happend while reading the program
		string error = string("Error reading program: ") + string(msg);
...
std::string CLMatrix3d::readMacro(void) {
	stringstream macro;
	
	macro << "#define REAL double" << endl;
...
double CLMatrix3d::maxNorm(bool includeRim) {
	...
	if(!includeRim)  this->setRim(buffer, 0.0);
